  ...
  return `<div class="p-6"><div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
    ${S.tags.map(t=>`<div class="glass-card rounded-2xl p-4 flex items-center justify-between">
      ...
      ${canManage ? `<div class="flex gap-0.5">
        ...
        <button class="btn-icon" style="color:#6b7280" onmouseenter="this.style.color='#f87171'" onmouseleave="this.style.color='#6b7280'" onclick="confirmDel('tag',${t.id},'${esc(t.name)}')"><span class="ms ms-sm">delete</span></button>
      </div>` : ''}
    </div>`).join('')}
