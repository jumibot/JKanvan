...
function mGroup(id) {
  ...
  showModal(`
    ...
    <form onsubmit="saveGroup(event,${id||'null'})">
      ...
      ${mfoot(g?'Guardar':'Crear grupo de tareas', !!g, `confirmDel('group',${id},'${esc(g?.name||'')}');closeM()`)}
    </form>`);
}
...
function mColMenu(id) {
  ...
  showModal(`
    ...
    <div class="flex flex-col gap-1">
      ...
      <button class="flex items-center gap-3 p-3 rounded-xl text-red-400 hover:bg-red-500/10 transition-all text-sm" onclick="confirmDel('group',${id},'${esc(g?.name||'')}')">
