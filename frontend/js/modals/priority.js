    ...
    ? S.priorities.map(pr => `
        <div class="flex items-center justify-between py-2.5 border-b" style="border-color:rgba(255,255,255,.06)">
          ...
          ${canManage ? `<div class="flex gap-0.5">
            ...
            <button class="btn-icon" style="color:#6b7280" onmouseenter="this.style.color='#f87171'" onmouseleave="this.style.color='#6b7280'" onclick="closeM();confirmDel('priority',${pr.id},'${pr.name}')"><span class="ms ms-sm">delete</span></button>
          </div>` : ''}
        </div>`).join('')
